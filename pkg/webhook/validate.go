...
func (v *AdmissionValidator) Handle(ctx context.Context, req types.Request) types.Response {
	...
	//validate DependencyMustExist
	if os.Getenv("ADMISSION_CONTROL_DEPENDENCIES") == "true" {
		mustexist := getDependencyMustExistLabel(availableLabels)
		logt.Info("Is dependency check labeled?", "isLabeled", mustexist)
		if mustexist == true {
			dependencies := getDependencies(req.AdmissionRequest.Kind.Kind, req.AdmissionRequest.Object.Raw)
			notfound := v.checkExistence(resourceNamespace, dependencies)
			if len(notfound) > 0 { //some dependencies not found
				var message string
				for _, item := range notfound {
					message += item.Kind + ":" + item.Name + "\n"
				}
				message = "following dependencies are not found in namespace " + resourceNamespace + ". \n" + message
				return types.Response{
					Response: &admissionv1beta1.AdmissionResponse{
						Allowed: false,
						Result: &metav1.Status{
		...
		if kind == imconfig.Kind {
			...
			for _, item := range immutables {
				...
				if spec[itemlower] != existingspec[itemlower] {
					...
					As a workaround, convert them to string and compare again */
